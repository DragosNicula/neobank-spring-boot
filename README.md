...
- ⚠️ **Centralized exception handling** via `@RestControllerAdvice`, mapping business-rule violations (duplicate usernames, insufficient funds, invalid IBANs) to clean, structured HTTP error responses
- 🖥️ **Full frontend** — routing, a reusable component library (inputs, buttons, cards, toggle), a React Context–based auth flow (login/logout reactively updating the navbar), protected routes, and a user profile page with account overview and transaction history
...
Built by **[Dragos Nicula](https://github.com/DragosNicula)** as part of a career transition from JavaScript/Vue.js/React.js to Java/Spring Boot full-stack development.
